...
{code:"CICNOTO",label:"CIC Noto",def:1,cat:"com",alloc:"fix",ord:8,pair:true,dows:[1,2,3,4]},
{code:"CICVDSR",label:"CIC/VD SR",def:2,cat:"com",alloc:"fix",ord:9,pair:true,dows:[1,2,3,4]},
...
var DISP=["CIC","NIC","NICSP","NICMIN","VD","222","CICNOTO","CICVDSR","VALID","PU","VDOM","CIECHI"];
...
function gen(year,month,users,gS,dayOv,eInd,inc,dRestr,locks,currentAsg){
  ...
  for(var d=1;d<=nd;d++){
    ...
    for(var fi=0;fi<FIX.length;fi++){
      ...
      if((ac==="CIECHI"||ac==="CICNOTO"||ac==="CICVDSR")&&!ov)en=false;
      // Weekday constraint (e.g. trasferta SR/Noto only Mon-Thu)
      if(act.dows&&act.dows.indexOf(dow)<0)en=false;
      if(!en){out[k][ac]=[];continue;}
      var nP=(ov&&typeof ov.slots==="number")?ov.slots:(gS[ac]!==undefined?gS[ac]:act.def);

      if(ac==="CIC"||ac==="VD"||ac==="CICNOTO"||ac==="CICVDSR"){out[k][ac]=sPairs(ac,nP);}
      else if(ac==="NICSP"){
    ...
    function rescueFill(ac){
      ...
      if((ac==="CIECHI"||ac==="CICNOTO"||ac==="CICVDSR")&&!ovR)enR=false;
      if(!enR)return;
...
function expHTML(yr,mo,nd,sn,asg,eInd,dOv,gS){
  ...
  for(var d=1;d<=nd;d++){var k=dk(yr,mo,d),we=isWE(yr,mo,d),hol=isH(yr,mo,d),off=we||!!hol;var da=asg[k]||{},di=eInd[k]||{};var bg=hol?"#fee2e2":we?"#f1f5f9":(d%2===0?"#fafafc":"#fff");var dc=hol?"#dc2626":we?"#94a3b8":"#334155";var lbl=dn(yr,mo,d)+" "+d+(hol?" "+hol:"");
    var cells=OPD.map(function(a){if(off)return'<td style="border:1px solid #bbb;background:'+bg+'"></td>';var ov=dOv[k]&&dOv[k][a.code];var en=ov?(ov.enabled!==false):true;if((a.code==="CIECHI"||a.code==="CICNOTO"||a.code==="CICVDSR")&&!ov)en=false;if(a.dows&&a.dows.indexOf(jd(yr,mo,d))<0)en=false;if(!en)return'<td style="border:1px solid #bbb;background:#fafafa;color:#ccc;text-align:center">\u2014</td>';
...
function VMese(p){
  ...
  var undoStack=useRef([]); // history of asg snapshots (manual grid edits only)
  var[undoCount,sUndoCount]=useState(0); // triggers button re-render
  // Clear undo history when month changes (undo is per-view)
  useEffect(function(){undoStack.current=[];sUndoCount(0);},[yr,mo]);
  ...
  // modDay: snapshot current asg into undo stack (max 10), then apply mutation
  var modDay=function(fn){
    sAS(function(prev){
      undoStack.current.push(JSON.parse(JSON.stringify(prev)));
      if(undoStack.current.length>10)undoStack.current.shift();
      return fn(JSON.parse(JSON.stringify(prev)));
    });
    sUndoCount(undoStack.current.length);
  };
  var doUndo=function(){
  ...
  return(<div>
    ...
    <div className="no-print" style={{display:"flex",gap:8,marginBottom:6,flexWrap:"wrap"}}>
      ...
      <button onClick={doUndo} disabled={undoCount===0} style={Object.assign({},cs.bs,{background:undoCount===0?"#f1f5f9":"#fff",color:undoCount===0?"#cbd5e1":"#334155",cursor:undoCount===0?"default":"pointer",borderColor:undoCount===0?"#e2e8f0":"#cbd5e1"})} title={undoCount?("Annulla ultima modifica ("+undoCount+" disponibili)"):"Nessuna modifica da annullare"}>{"\u21A9"} Annulla{undoCount>0?" ("+undoCount+")":""}</button>
          ...
          {days.map(function(d){
            ...
            return(<tr key={d} style={{background:bg}}>
              ...
              {OPD.map(function(a){
                ...
                if((a.code==="CIECHI"||a.code==="CICNOTO"||a.code==="CICVDSR")&&!ov)en=false;if(a.dows&&a.dows.indexOf(jd(yr,mo,d))<0)en=false;
...
function VSlot(p){
  ...
  var QK=["CIECHI","NICSP","NICMIN","CICNOTO","CICVDSR"];
  return(<div>
    ...
    <div style={{overflowX:"auto",marginBottom:16}}>
      <table style={{borderCollapse:"collapse"}}><thead><tr><th style={cs.h}>Giorno</th>
        ...
        <tbody>{wd.map(function(d){var k=dk(yr,mo,d);var dow=jd(yr,mo,d);return(<tr key={d}><td style={cs.c}>{dn(yr,mo,d)} {d}</td>
          {QK.map(function(code){var actQ=ACT.find(function(x){return x.code===code;});var dowBlocked=actQ&&actQ.dows&&actQ.dows.indexOf(dow)<0;
            if(dowBlocked)return(<td key={code} style={Object.assign({},cs.c,{textAlign:"center",background:"#f8f8f8"})}><span style={{color:"#cbd5e1",fontSize:10}}>{"\u2014"}</span></td>);
            var ov=dOv[k]&&dOv[k][code];var en=ov?(ov.enabled!==false):true;if(code==="CIECHI"||code==="CICNOTO"||code==="CICVDSR"){en=ov?(ov.enabled!==false):false;}
